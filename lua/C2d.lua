...
function cc.DegreesToRadians(angle)
	return angle*0.01745329252;
end

function cc.RadiansToDegrees(angle)
	return angle* 57.29577951;
end

function cc.pToVec3(p)
	return Vector3(p.x, 0, p.y)
end

function cc.vec2ToP(vec2)
	return cc.p(vec2.x, vec2.y)
end

function cc.vec3ToP(vec3)
	return cc.p(vec3.x, vec3.z)
end

function cc.GetDeltaOnSegment(p1, p2, dis, minDisBetweenP2)
	
	local p1p2Dis = cc.pGetDistance(p1, p2)
	local moveDis = dis
	
	if p1p2Dis < dis + minDisBetweenP2 then
		
		if minDisBetweenP2 > p1p2Dis then
			moveDis = 0
		else
			moveDis = p1p2Dis - minDisBetweenP2
		end
	end
	local p3 = cc.pSub(p2, p1)
	local p4 = cc.pNormalize(p3)
	local p5 = cc.pMul(p4, moveDis)
		
	return moveDis, p5, p4
end

function cc.GetDeltaOnSegment_1(p1, p2, leftDis)
	
	local p1p2Dis = cc.pGetDistance(p1, p2)
	local moveDis = dis
	
	if p1p2Dis < leftDis then
		moveDis = 0
	else
		moveDis = p1p2Dis - leftDis
	end
	local p3 = cc.pSub(p2, p1)
	local p4 = cc.pNormalize(p3)
	local p5 = cc.pMul(p4, moveDis)
...
function cc.p3(_x,_y,_z)
    return { x = _x, y = _y, z = _z }
end

function cc.p3Add(pt1, pt2)
    return {x = pt1.x + pt2.x , y = pt1.y + pt2.y, z = pt1.z + pt2.z }
end

function cc.p3Sub(pt1,pt2)
    return {x = pt1.x - pt2.x , y = pt1.y - pt2.y, z = pt1.z - pt2.z }
end

function cc.p3Mul(pt1,factor)
